...
def days_ago(n, hour=0, minute=0, second=0, microsecond=0):
    return datetime.combine(
        datetime.now(dt_timezone.utc) - timedelta(days=n),
        time(hour, minute, second, microsecond, tzinfo=dt_timezone.utc),
    )
...
def ckan_datstore_loader_failure(context):
    ...
    api_key = context['params'].get('ckan_config', {}).get('api_key')
    site_url = context['params'].get('ckan_config', {}).get('site_url')
    err_msg = getattr(exception, 'err', None) or getattr(exception, 'value', None) or str(exception)
    logging.error(err_msg)
    status_dict = {
            'res_id': resource_dict.get('ckan_resource_id'),
            'state': 'error',
            'message': getattr(exception, 'value', str(exception)),
            'error': err_msg
        }
