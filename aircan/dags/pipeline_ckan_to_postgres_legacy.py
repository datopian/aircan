...
from aircan.dependencies_legacy.postgres_loader import (
    load_csv_to_postgres_via_copy,
    delete_index,
    restore_indexes_and_set_datastore_active
    )

from aircan.dependencies_legacy.utils import (
    days_ago, get_connection,
    to_bool, ckan_datstore_loader_failure)

from aircan.dependencies_legacy.api_loader import (
    fetch_and_read,
    compare_schema,
    check_append_enabled,
...
DATASETS_SKIP_UNIQUE_KEY_CHECK = Variable.get("NG_DATASETS_SKIP_UNIQUE_KEY_CHECK", "")
...
@task(task_id='fetch_resource_data')
def fetch_resource_data(**context):
    """
    #### Fetch and read task
...
@task.branch(task_id='check_schema')
...
@task(task_id='create_datastore_table')
def create_datastore_table_task(**context):
    """
    #### create new datastore table
    This task deletes the existing ckan datastore table and create new table based
    on detected schema in 'fetch_resource_data'.
    """
    ti = context['ti']
...
@task(task_id='push_data_into_datastore', trigger_rule='none_failed_min_one_success')
def push_data_into_datastore(**context):
    """
    #### Push data into datastore
    This task pushes the data into datastore on newly created or existing
    datastore table.
    """
...
@task(task_id='generate_file_and_load_to_GCP', trigger_rule='none_failed_min_one_success')
def generate_file_and_push_to_GCP(**context):
    """
...
with DAG(
    dag_id='pipeline_ckan_to_postgres_legacy',
    default_args=args,
    ...
    # [SET WORKFLOW]
    fetch_resource_data_task = fetch_resource_data()
    check_schema_task = check_schema()
    create_table_task = create_datastore_table_task()
    push_task = push_data_into_datastore()
    gcp_task = generate_file_and_push_to_GCP()

    fetch_resource_data_task >> check_schema_task >> [create_table_task, push_task]
    create_table_task >> push_task
    push_task >> gcp_task
    # [END WORKFLOW]
