...
from aircan.dependencies_legacy.s3_uploader import s3Uploader

from aircan.dependencies_legacy.postgres_loader import aircan_status_update
...
def fetch_and_read(resource_dict, site_url, api_key):
    ...
    try:
        ...
        resource = describe(path=resource_tmp_file.name, type="resource", encoding="utf-8")
        status_dict = { 
        ...
        return {'sucess': True, 'resource': resource.to_descriptor(), 'resource_tmp_file': resource_tmp_file.name}

    except Exception as err:
        raise AirflowCKANException(
             'Failed to fetch data file from {0}.'.format(resource_dict['path']), str(err))

def fetch_data_dictionary(res_id, site_url, ckan_api_key):
    ...
    for attempt in range(retry_attempts):
        logging.info('Attempt {0} to fetch data dictionary'.format(attempt))
        response = requests.get(url, headers=headers, params=params)
        if response.status_code in [200, 404]:
            return response
        elif attempt == retry_attempts - 1:
            raise AirflowCKANException('Failed to fetch data dictionary for {0}'.format(res_id))
...
def compare_schema(site_url, ckan_api_key, res_dict, schema):
    ...
    try:
        response = fetch_data_dictionary(res_id, site_url, ckan_api_key)    
        ...
        if response.status_code == 200:
            ...
            logging.info('logging old and new schema columns')
            logging.info(old_schema_columns)
            logging.info(new_schema_columns)
